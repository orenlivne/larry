### Tarrasch Countergambit
1. d4 d5  
2. c4 c5  

├── 3. dxc5 (Black: 3... Nf6) — The Grab  
│ A direct pawn grab; Black counterplays with piece play to regain the pawn on c5.  
│  
├── 3. cxd5 (Black: 3... Qxd5) — The Trade  
│ Early simplification in the center. When he takes on c5, play ...Nbd7 and take back with the N  
│ to prevent a3–b4 tempi. Clarifies the center and leads to simplified tension and piece play.  
│  
├── 3. Nc3 (Black: 3... cxd4) — Tarrasch Hopeful  
│ Tries to transpose to the main Tarrasch with 3...e6, but we surprise with a pawn sac on d5  
│ for initiative (like Dubov–Tarrasch). Play 7...Qc7 to avoid a Q-trade.  
│ On 9. Qd3 play 9...Bc5! Be3 Bb4! — provoking White to block his e-pawn & development.  
│  
├── 3. e3 (Black: 3... Nf6) — Solid / Colle  
│ Reverse Catalan structure; Black develops flexibly and can mirror Qb3 → ...Qb6.  
│  
└── 3. Nf3 (Black: 3... cxd4) — Main Line (Development)  
  4. cxd5 Qd5  

  ├── 5. Qxd4 (Black: 5... Nf6) — Symmetrical Nakamura  
  │ Basic plan: trade queens early, then develop with ...a6, ...e5, ...Nc6.  
  │ Nakamura’s dynamic idea: expand with ...a5 and ...Ra8–a5 for activity.  
  │  
  └── 5. Nc3 Qa5  
    6. Nd4 Nf6  

    ├── 7. Nb3 (Black: 7... Qc7) — Pressure System  
    │ Retreat to target c4/d5 while guarding f4/e5.  
    │ Black’s plan: ...e5, ...Nc6, ...Be6.  
    │  
    │ ├── 8. Nb5 (Black: 8... Qe5) - Counterattack line
    │ │ A sharp line of counterattacks; Black often reroutes the Q back to c7 later.  
    │ │ White may even sacrifice a pawn with e6!? for dynamic play.  
    │ │  
    │ └── 8. e4 Nc6  
    │   Continuing central expansion. Typical follow-ups: 9. Bb5, Bg5, Be3, or Be2.  
    │  
    │   ├── 9. Bb5 (Black: 9... Bd7 + ...e6) — Classic Sicilian
    │   │ Only case for ...e6 — prevents Nd5; then ...Bb4 → ...Nxe4.  
    │   │ Eventually gives back the pawn; king often finds safety on f7 but we can give up ...h5 Qxg7 -> ...000!
    │   │  
    │   ├── 9. Bg5 (Black: 9... g6) — Fianchetto Anyway  
    │   │ Concrete play; compensates dynamically for the d5 weakness.  
    │   │  
    │   ├── 9. Be3 (Black: 9... e5) — Sveshnikov Style  
    │   │ Dynamic central counter; same thematic d5-weakness compensation.  
    │   │  
    │   └── 9. Be2 (Black: ...e5) — Classical  
    │    Classical development (...Be7, ...Be6, ...Nc6–d4).  
    │    May sacrifice a pawn for a dominating knight on d4.  
    │  
    ├── 7. g3 — "Kingside Fianchetto"  
    │ (Black: 7... e5, flexible replies — ...Qc7 / ...Nc6)  
    │ White aims for long-term kingside stability; Black counters with  
    │ queenside activity or central breaks.  
    │  
    ├── 7. a3 — "Prophylaxis"  
    │ (Black: flexible — often ...a4 or ...Be6)  
    │ Prevents ...Bb4 / ...Na4 and prepares b4; Black answers with  
    │ fast queenside expansion.  
    │  
    └── 7. Bd2 — "The Direct Line"  
      (Black: 7... e5, and often ...Qb6 / ...Nc6)  
      Straightforward development aiming to neutralize Black’s queen activity;  
      play revolves around maintaining central tension and piece activity. Play Ns to d4, f4 to compensate for 2Bs.
